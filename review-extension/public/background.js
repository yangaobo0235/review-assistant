...
importScripts("image-normalization.js");
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type !== "FETCH_IMAGE_ASSET") return;

  console.info("[ReviewAgent][background] image fetch start");
  fetch(message.url, { credentials: "include" })
    .then(async (response) => {
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const blob = await globalThis.ReviewImageNormalization.readResponseBlobWithLimit(response);
