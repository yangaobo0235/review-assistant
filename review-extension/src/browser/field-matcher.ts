interface MatchCandidate { value: string; section?: string; label: string; context: string; }


  const expectedSection = (field: string) => {
  ...
  const select = (field: string, aliases: string[], candidates: MatchCandidate[]) => {
  ...
  export const ReviewFieldMatcher = { select };
