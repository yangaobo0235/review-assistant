...
import type { Evidence, MaterialCompletenessReport, PageData, PageFillAction, QrCheck, ReviewResponse, ReviewStep, SubjectEvidenceRequirement } from "../types/review";
...
function fieldFromStep(step: ReviewStep) { return step.page_field || (step.step_id.startsWith("FIELD-") && !step.step_id.startsWith("FIELD-DOM-") ? step.step_id.slice(6) : null); }
function stepTitle(step: ReviewStep) { const field = fieldFromStep(step); const labels: Record<string, string> = { "old_vehicle.affiliation": "报废车挂靠", "new_vehicle.affiliation": "新车挂靠" }; return step.step_id.startsWith("FIELD-DOM-") ? step.label : field ? labels[field] || fieldLabel(field) : step.label; }
...
function StepDetail({ step, policy, pageData, qrChecks, affiliationActions, affiliationFilled, disabled, onFocusImage, onFill, onFillAffiliation, onChoose, materialReport }: { step: ReviewStep; policy?: ReviewStep; pageData?: PageData | null; qrChecks: QrCheck[]; affiliationActions: PageFillAction[]; affiliationFilled: boolean; disabled: boolean; onFocusImage: (id: string) => Promise<void>; materialReport?: MaterialCompletenessReport | null; onFill: (step: ReviewStep, value: string) => Promise<void>; onFillAffiliation: () => void; onChoose: (step: ReviewStep, decision: Decision) => void }) {
