...
前端打开审核页后，Adapter 先生成页面指纹并采集字段及材料索引。用户点击开始审核时，Hook 先用页面字段和图片清单创建流式任务，再用两个 Worker 逐张读取、压缩和上传图片。后端每收到一张图片就将它放入公平识别队列；全部上传关闭且识别结束后，统一 LangGraph 复用单图结果完成材料、二维码、比较、规则和建议汇总。前端轮询任务快照并按任务类型渲染；用户确认后，Action Controller 再次校验页面指纹和目标控件，执行白名单动作并回读。并发、状态和清理细节见[审核流水线、并发与任务生命周期](审核流水线.md)。
