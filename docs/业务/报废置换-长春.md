...
本文档是长春 Profile 的业务基线。代码实现分别位于后端 `app/businesses/profiles.py`、`app/businesses/replacement_policies.py`、`app/businesses/rules/replacement_policy.py`、`app/businesses/completeness.py`、`app/businesses/rules/affiliation.py`，以及前端 `src/adapters`、`src/browser`、`src/components` 和 `src/session`。长春与青岛共用审核流程和材料策略，但政策窗口、产地范围和能力 ID 必须保持地区一致。旧车 VIN、发票/新车 VIN 组合核验、手机号和材料展示遵循通用字段协议：二维码官网 VIN 为最高标准，行驶证/登记证只核对后 8 位；发票代码/号码、新车 VIN/OCR VIN 先校验页面双值一致再比材料；手机号只认页面值；材料任务只显示是否齐全。
...
政策 ID 为 `scrap_replacement_changchun`，版本 `1.0`。三项政策检查读取与青岛相同的指定图片字段。**2026-09-21 起两个日期的窗口与青岛统一为 2026 全年**；**2026-09-21 起发票产地也改为同口径的关键词匹配**。两地现在只在关键词上不同（本地地名），规则形状完全一致：

1. **新车发票日期**：`invoice.invoice_date` 必须在 **2026-01-01 至 2026-12-31（含边界）**。缺失、格式无效、多个冲突值或 OCR 不确定为 `INSUFFICIENT`。
2. **回收证明交车日期**：`old_vehicle.recycle_date` 必须在 **2026-01-01 至 2026-12-31（含边界）**。缺失、格式无效或证据不唯一为 `INSUFFICIENT`。

**超出政策窗口交给人工复核，不自动判不通过**（2026-09-21 起）。窗口边上的一天之差（材料出具时间、节假日顺延）要靠人工判断，自动否决会误伤，因此超范围与"读不到日期"同样是 `INSUFFICIENT`，理由里写清超出的是哪个范围。
3. **新车发票产地**：`new_vehicle.origin` 必须规范化后包含“长春”关键词，允许“长春”“长春市”“吉林省长春市”等包含该关键词的值。无法取得唯一值为 `INSUFFICIENT`，明确不包含关键词为 `CONFLICT`。青岛自 2026-09-21 起也是这个口径（关键词换成“青岛”），两地的宽松程度必须保持一致——`tests/test_replacement_policy_checks.py::test_origin_matches_by_keyword_in_every_region` 锁住这一点。
...
| 2026-09-21 | 发票产地与青岛统一为关键词匹配 | 长春规则本身没变，变的是青岛向长春看齐。此前青岛按完全匹配，同一张发票换个地区审核结论会不同。政策 `version` 未动，理由同日期窗口那条。 |
...
- `BusinessRuleRegistry`（业务规则注册表）：注册 `changchun_replacement_policy`（长春置换政策）、`affiliation_subject`（挂靠主体关系）和 `verify_invoice`（发票一键验真）。
...
| 材料清单 | 共享 `app/businesses/material_policies.py`、`app/businesses/completeness.py` | `MaterialCompleteness.tsx` | 图片、类型、页码、读取失败 |
| 政策日期/产地 | `app/businesses/packs/scrap_replacement.py` | 政策任务 Renderer | 长春日期窗口、产地关键词 |
| 主体关系 | 共享 `app/businesses/rules/affiliation.py` | 主体证据组件 | 身份证、营业执照、法人 |
