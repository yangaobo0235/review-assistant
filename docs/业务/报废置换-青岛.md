...
3. **新车发票产地**：读取 `new_vehicle.origin`，来源必须是新车发票图片。规范化后**包含“青岛”关键词即通过**，因此“青岛”“青岛市”“山东省青岛市”“中国青岛”“产地：青岛”都算符合；明确不含关键词（如“长春”）为 `CONFLICT`，无法取得唯一值为 `INSUFFICIENT`。

   **2026-09-21 起由完全匹配改为关键词匹配，与长春同口径。** 发票产地栏的写法不固定，完全匹配会把“中国青岛”“产地：青岛”这类规范写法之外的值一律判成冲突；两个地区的宽松程度必须一致，否则同一张发票换个地区审核结论就不同。`allowed_origins` 仍保留规范写法清单，但关键词命中在先，它不再单独决定结论。
...
| 2026-09-21 | 新车发票产地改为关键词匹配 | 由"完全匹配三个允许值"改为"**包含『青岛』即通过**"，与长春同口径。受影响的值：`中国青岛`、`产地：青岛` 由 `CONFLICT` 变为 `MATCH`；`长春` 仍为 `CONFLICT`；读不出来的值仍为 `INSUFFICIENT`。政策 `version` 未动，理由同日期窗口那条。两地同口径由 `tests/test_replacement_policy_checks.py::test_origin_matches_by_keyword_in_every_region` 锁定。 |
